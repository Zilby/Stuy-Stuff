...
public class Canvas extends JPanel {
	...
	if (drawing) {
	    ...
	    for (; i < col - 2; i+=3){
		if(tmpline.calculateDot(i) < 0){
		    g2.drawLine( (int)tmpline.getX(i), (int)tmpline.getY(i), 
				 (int)tmpline.getX(i+1), (int)tmpline.getY(i+1));
		    g2.drawLine( (int)tmpline.getX(i+1), (int)tmpline.getY(i+1), 
				 (int)tmpline.getX(i+2), (int)tmpline.getY(i+2));
		    g2.drawLine( (int)tmpline.getX(i+2), (int)tmpline.getY(i+2), 
				 (int)tmpline.getX(i), (int)tmpline.getY(i));
		}
	    }
	}
	...
	try{
	    for (i=0; i < col - 2; i+=3) {
		if(edges.calculateDot(i) < 0){
		    g2.drawLine( (int)edges.getX(i), (int)edges.getY(i), 
				 (int)edges.getX(i+1), (int)edges.getY(i+1));
		    g2.drawLine( (int)edges.getX(i+1), (int)edges.getY(i+1), 
				 (int)edges.getX(i+2), (int)edges.getY(i+2));
		    g2.drawLine( (int)edges.getX(i+2), (int)edges.getY(i+2), 
				 (int)edges.getX(i), (int)edges.getY(i));
		}
	    }
	}
	catch(ArrayIndexOutOfBoundsException e){
	    System.out.println("attempted index:" + i);
	    System.out.println("terminal index of edges:" + edges.getLastCol());
	    System.out.println("terminal index of tmpline:"+tmpline.getLastCol());
	}
    ...
    /*======== public void setDrawing() ==========
      ...
      if mode == gui.SPHERE_MODE_M
          adds a wire frame sphere mesh to the tmpline EdgeMatrix
      if mode == gui.TORUS_MODE_M
          adds a wire frame torus mesh to the tmpline EdgeMatrix
      if mode == gui.BOX_MODE_M
          adds a wire frame rectangular prism to the tmpline EdgeMatrix

      ====================*/    
    public void setDrawing(int x0, int y0, int x1, int y1, int x2, int y2, int rx, int ry, int mode) {
	
	if ( mode == gui.LINE_MODE )
	    tmpline.addPolygon(x0, y0, 0, x1, y1, 0, x1, y1, 0);
	
	else if ( mode == gui.CIRCLE_MODE ) 
	    tmpline.addCircle( x0, y0, x1 );

	else if ( mode == gui.HERMITE_MODE || mode == gui.BEZIER_MODE ) {
	    	    
	    tmpline.addEdge(x0, y0, 0, x1, y1, 0);
	    tmpline.addEdge(x2, y2, 0, rx, ry, 0);
	    tmpline.addCurve( x0, y0, x1, y1, x2, y2, rx, ry, mode );
	}

	else if ( mode == gui.SPHERE_MODE ) {
	    tmpline.addSphere( x0, y0, x1);
	}

	else if ( mode == gui.TORUS_MODE ) {
	    tmpline.addTorus( x0, y0, x1, y1 );
	}

	else if ( mode == gui.BOX_MODE) {
	    tmpline.addBox( x0, y0, x1, y1, x2, y2 );
	}
    ...
    /*======== public void addSphere() ==========
      ...
      Calls addSphere on the permamnent EdgeMatrix,
      updates the graphics

      03/28/12 18:53:07
      jonalf
      ====================*/
    public void addSphere(int cx, int cy, int r) {
	
	edges.addSphere( cx, cy, r );
	this.update( this.getGraphics() );
    }
    ...
    /*======== public void addTorus() ==========
      ...
      Calls addTorus on the permamnent EdgeMatrix,
      updates the graphics      
      
      03/28/12 18:54:13
      jonalf
      ====================*/
    public void addTorus(int cx, int cy, int r1, int r2) {
	
	edges.addTorus( cx, cy, r1, r2 );
	this.update( this.getGraphics() );
    }
    ...
    /*======== public void addBox() ==========
      ...
      Calls addBox on the permamnent EdgeMatrix,
      updates the graphics            
      
      03/28/12 18:54:49
      jonalf
      ====================*/
    public void addBox(int x, int y, int z, int width, int height, int depth) {
	
	edges.addBox( x, y, z, width, height, depth );
	this.update( this.getGraphics() );
    }
