...
def calc_surface_normal(points, index):
    p0 = points[index]
    p1 = points[index+1]
    p2 = points[index+2]
    a = [p1[0]-p0[0], p1[1]-p0[1], p1[2]-p0[2]]
    b = [p2[0]-p0[0], p2[1]-p0[1], p1[2]-p0[2]]
    res = [a[1]*b[2]-a[2]*b[1],a[2]*b[0]-a[0]*b[2],a[0]*b[1]-a[1]*b[0]]
    return res

def calc_dot_product(n, v):
    cos = n[0]*v[0]+n[1]*v[1]+n[2]*v[2]
    return cos

def is_visible(points, index, v):
    n = calc_surface_normal(points, index)
    cos = calc_dot_product(n, v)
    return cos < 0
...
def draw_polygons( points, screen, color ):
    if len( points ) < 3:
        print "There aren't enough points to draw a polygon!"
        
    p = 0
    while p < len( points ):
        v = [0,0,-1]
        if is_visible(points, p, v):
...
def add_box( points, x, y, z, width, height, depth ):
    ...
    add_polygon( points, x, y, z, 
                         x, y1, z, 
                         x1, y1, z)
    add_polygon( points, x1, y1, z, 
                         x1, y, z, 
                         x, y, z)
    add_polygon( points, x1, y, z, 
                         x1, y1, z, 
                         x1, y1, z1)
    add_polygon( points, x1, y1, z1, 
                         x1, y, z1, 
                         x1, y, z) 
    add_polygon( points, x1, y, z1, 
                         x1, y1, z1, 
                         x, y1, z1) 
    add_polygon( points, x, y1, z1, 
                         x, y, z1, 
                         x1, y, z1) 
    add_polygon( points, x, y, z1, 
                         x, y1, z1, 
                         x, y1, z)
    add_polygon( points, x, y1, z, 
                         x, y, z, 
                         x, y, z1)   
    add_polygon( points, x, y, z1, 
                         x, y, z, 
...
def add_torus( points, cx, cy, cz, r0, r1, step ):
    ...
    while lat <= lat_stop:
        ...
        while longt < longt_stop:
            ...
            add_polygon( points, 
                    temp[index][0], temp[index][1], temp[index][2], 
                    temp[(index)%mod_len+1][0], temp[(index)%mod_len+1][1], temp[(index)%mod_len+1][2], 
                    temp[(index+num_steps)%mod_len+1][0], temp[(index+num_steps)%mod_len+1][1], temp[(index+num_steps)%mod_len+1][2] )
            add_polygon( points, 
                    temp[index][0], temp[index][1], temp[index][2], 
                    temp[(index+num_steps)%mod_len+1][0], temp[(index+num_steps)%mod_len+1][1], temp[(index+num_steps)%mod_len+1][2], 
                    temp[(index+num_steps)%mod_len][0], temp[(index+num_steps)%mod_len][1], temp[(index+num_steps)%mod_len][2] )
