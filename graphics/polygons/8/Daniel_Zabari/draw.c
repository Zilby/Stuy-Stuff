...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
	int i=0;
	for (i=0;i<(polygons->lastcol);	i+=2){
		print_matrix(polygons);
		draw_line(polygons->m[i][0],polygons->m[i][1],polygons->m[i+1][0],polygons->m[i+1][1],s,c);
		printf("first\n");
	    draw_line(polygons->m[i+1][0],polygons->m[i+1][1],polygons->m[i+2][0],polygons->m[i+2][1],s,c);
        printf("second\n");
		draw_line(polygons->m[i][0],polygons->m[i][1],polygons->m[i+2][0],polygons->m[i+2][1],s,c);
		printf("third\n");
	}
	printf("done\n");
}
...
void add_box( struct matrix *points,
  ...
  z2 = z - depth;
//Left Face
	//Top Front Left, Bottom Front Left, Bottom Back Left
  add_polygon( points, 
	    x, y, z, 
	    x, y2, z ,
		x, y2 ,z2);
	//Top Front Left, Top Back Left, Bottom Back Left
  add_polygon( points, 
	    x, y, z, 
	    x, y, z2 ,
		x,y2,z2);
//Front Face
    //Top Front Left, Bottom Front Left, Bottom Front Right
  add_polygon( points,
        x, y, z,
        x, y2, z,
        x2,y2,z);
	//Top Front Left, Bottom Front Right, Top Front Right
  add_polygon( points, 
	    x, y, z, 
	    x2, y2, z, 
		x2, y, z);
//Top Face
	//Top Front Left, Top Front Right,Top Back Left 
  add_polygon( points, 
	    x, y, z, 
	    x2, y, z,
		x,y,z2 );
	//Top Front Right, Top Back Right, Top Back Left
  add_polygon( points, 
	    x2, y, z, 
	    x2, y, z2, 
		x,y,z2);
//Bottom Face
    //Bottom Front Left, Bottom Front Right,Bottom Back Left 
  add_polygon( points,
        x, y2, z,
        x2, y2, z,
        x,y2,z2 );
    //Bottom Front Right, Bottom Back Right, Bottom Back Left
  add_polygon( points,
        x2, y2, z,
        x2, y2, z2,
        x,y2,z2);
//Back Face
    //Top Back Left, Bottom Back Left, Bottom Back Right
  add_polygon( points,
        x, y, z2,
        x, y2, z2,
        x2,y2,z2);
    //Top Back Left, Bottom Back Right, Top Back Right
  add_polygon( points,
        x, y, z2,
        x2, y2, z2,
        x2, y, z2);
//Right Face
    //Top Front Right, Bottom Front Right, Bottom Back Right
  add_polygon( points,
        x2, y, z,
        x2, y2, z,
        x2, y2 ,z2);
    //Top Front Right, Top Back Right, Bottom Back Right
  add_polygon( points,
        x2, y, z,
        x2, y, z2,
        x2,y2,z2);
