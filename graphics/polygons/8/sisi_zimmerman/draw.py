...
def get_normal( x0, y0, z0, x1, y1, z1, x2, y2, z2 ):
    ...
    B = [x0-x2, y0-y2, z0-z2]
    norm = []
    norm.append(A[1]*b[2] - A[2]*B[1])
    norm.append(A[2]*b[0] - A[0]*B[2])
    norm.append(A[0]*b[1] - A[1]*B[0])
    return norm

def dot_product(view,normal):
    return view[0]*normal[0] + view[1]*normal[1] + view[2]*normal[2]
...
def draw_polygons( points, screen, color ):
    ...
    while p < len( points ) - 2:
        ...
        draw_line( screen, matrix[p][0], matrix[p][1],
                   matrix[p+1][0], matrix[p+1][1], [255,0,0] )
        draw_line( screen, matrix[p+1][0], matrix[p+1][1],
                   matrix[p+2][0], matrix[p+2][1], [0,255,0] )
        draw_line( screen, matrix[p+2][0], matrix[p+2][1],
                   matrix[p][0], matrix[p][1], [0,0,255] )
        p+= 3
...
def add_box( points, x, y, z, width, height, depth ):
    ...
    add_polygon( points, x1, y1, z1, x1, y1, z, x1, y, z)
    ...
    while lat < lat_stop:
        ...
        while longt < longt_stop:
            ...
            add_polygon(points,
                        ...
                        temp[index][0+n], temp[index][1+n], temp[index][2+n],
                        temp[index][0+1+n], temp[index][1+1+n], temp[index][2+1+n])
            add_polygon(points,
                        temp[index][0], temp[index][1], temp[index][2],
                        temp[index][0+1+n], temp[index][1+1+n], temp[index][2+1+n],
                        temp[index][0+1], temp[index][1+1], temp[index][2+1])
