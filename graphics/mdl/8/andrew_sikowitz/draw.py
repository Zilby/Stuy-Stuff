...
def get_cross(A, B): #(jk-kj)i + (ki-ik)j + (ij-ji)k
    x = []
    x.append(A[1]*B[2] - A[2]*B[1])
    x.append(A[2]*B[0] - A[0]*B[2])
    x.append(A[0]*B[1] - A[1]*B[0])
    return x

def is_visible(p): #p is polygon
    V = [0,0,-1]
    A = [p[1][0]-p[0][0], p[1][1]-p[0][1], p[1][2]-p[0][2]]
    B = [p[2][0]-p[0][0], p[2][1]-p[0][1], p[2][2]-p[0][2]]
    N = get_cross(A,B)
    cosine = N[0]*V[0] + N[1]*V[1] + N[2]*V[2] #Can ignore division by product of norms--looking at roots only
    return cosine < 0
...
def draw_polygons( matrix, screen, color ):
    ...
    p = 0
    while p < len(matrix) - 1: #Should never be leftover 1/2 points
        if is_visible(matrix[p:p+3]):
