...
# Viewable Function
# Determines if the face is indeed viewable
def viewable( x0, y0, z0, x1, y1, z1, x2, y2, z2 ):
    ...
    # Surface Normal
    surf_norm = [ ( A[1] * B[2] ) - ( A[2] * B[1] ),
                  ( A[2] * B[0] ) - ( A[0] * B[2] ),
                  ( A[0] * B[1] ) - ( A[1] * B[0] ) ]

    # Determine Visibility
    norm = [0, 0, -1]
    a = surf_norm[0] * norm[0]
    b = surf_norm[1] * norm[1]
    c = surf_norm[2] * norm[2]
...
def draw_polygons( points, screen, color ):
    ...
    while ( x < len(points) - 1 ):
        if ( viewable( points[x][0], points[x][1], points[x][2],
