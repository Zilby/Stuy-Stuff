  ...
  for command in commands:
    if command[0] == "frames":
      if num_frames == -1:
        ...
        print "You can only set number of frames once"
        break
    ...
    if command[0] == "vary":
      if num_frames == -1:
        print "You must set number of frames before calling vary"
        break
...
def second_pass( commands, num_frames ):
    ...
    for command in commands:
      if command[0] == "vary":
        ...
        end_value = command[5]
        step = float(end_value - start_value)/(end_frame - start_frame)
        val = start_value
        for frame in range(start_frame):
          knobs[frame][knob] = start_value
        for frame in range(start_frame, end_frame):
          knobs[frame][knob] = start_value + step*(frame-start_frame)
        for frame in range(end_frame, num_frames):
          knobs[frame][knob] = end_value
...
def run(filename):
    ...
    for frame in range(num_frames): 
      print "FRAME: " + str(frame) + " KNOBS: " + str(knobs[frame])
      stack = [ tmp ]
      screen = new_screen()  
      ...
      for command in commands:
          ...
          if command[0] == "sphere":
              m = []
              add_sphere(m, command[1], command[2], command[3], command[4], 3)
          ...
          if command[0] == "move":                
              ...
              yval = command[2]
              zval = command[3]
              if is_anim and len(command) == 5 and command[4] in knobs[frame]:
          ...
          if command[0] == "rotate":
              ...
              if is_anim and len(command) == 4 and command[3] in knobs[frame]:
                angle *= knobs[frame][command[3]]
