...
def second_pass(commands, num_frames, basename):
    ...
    for frame in range(len(frames)):    
        runFrame(commands, screen, frames[frame], basename, frame)
...
def runFrame(commands, screen, knobs = [], basename = False, frameNum = 0):
    color = [255, 255, 255]
    tmp = new_matrix()
    ident(tmp)
    stack = [tmp]
        ...
        for knob in knobs:
            if knob in cmd:
                ...
                for arg in range(len(cmd)):
                    if not isinstance(cmd[arg], str):
                        cmd[arg] = cmd[arg] * mult
                cmd = tuple(cmd)
        print cmd
...
>>>>>>> origin/master
        ...
        if command[0] == "sphere":
            m = []
            add_sphere(m, command[1], command[2], command[3], command[4])
            matrix_mult(stack[-1], m)
            draw_polygons( m, screen, color )

        if command[0] == "torus":
            m = []
            add_torus(m, command[1], command[2], command[3], command[4], command[5])
            matrix_mult(stack[-1], m)
            draw_polygons( m, screen, color )
...
=======
        ...
        if cmd[0] == "sphere":
            m = []
            if cmd[5] == None:
                add_sphere(m, cmd[1], cmd[2], cmd[3], cmd[4])
            else:
                add_sphere(m, *cmd[1:])
            matrix_mult(stack[-1], m)
            draw_polygons(m, screen, color)
        ...
        if cmd[0] == "planet":
            ...
            if cmd[23] == None:
                ...
                cmd[23] = 15
                cmd = tuple(cmd)
            add_sphere(m, cmd[1], cmd[2], cmd[3], cmd[4], cmd[23])
            matrix_mult(stack[-1], m)
            draw_planet_polygons(m, screen, *cmd[5:])

        if cmd[0] == "torus":
            m = []
            add_torus(m, cmd[1], cmd[2], cmd[3], cmd[4], cmd[5])
            matrix_mult(stack[-1], m)
            draw_polygons( m, screen, color )
        ...
        if cmd[0] == "bezier":
            m = []
            add_curve(m, cmd[1], cmd[2], cmd[3], cmd[4], cmd[5], cmd[6], cmd[7], cmd[8], .05, 'bezier')
            matrix_mult(stack[-1], m)
            draw_lines( m, screen, color )

        if cmd[0] == "hermite":
            m = []
            add_curve(m, cmd[1], cmd[2], cmd[3], cmd[4], cmd[5], cmd[6], cmd[7], cmd[8], .05, 'hermite')
            matrix_mult(stack[-1], m)
            draw_lines( m, screen, color )

        if cmd[0] == "circle":
            m = []
            add_circle(m, cmd[1], cmd[2], cmd[3], cmd[4], .05)
            matrix_mult(stack[-1], m)
            draw_lines( m, screen, color )
...
=======
        ...
        if cmd[0] == "light":
            LIGHTS.append((cmd[1], cmd[2], cmd[3], cmd[4], cmd[5], cmd[6]))

        if cmd[0] == "ambient":
            AMBIENT_LIGHT[0] = cmd[1]
            AMBIENT_LIGHT[1] = cmd[2]
            AMBIENT_LIGHT[2] = cmd[3]

        if cmd[0] == "diffuse":
            DIFFUSE_LIGHT[0] = cmd[1]
            DIFFUSE_LIGHT[1] = cmd[2]
            DIFFUSE_LIGHT[2] = cmd[3]

        if cmd[0] == "specular":
            if len(cmd) < 4:
                SPECULAR_CONSTANT = cmd[1]
            else:
                SPECULAR_LIGHT[0] = cmd[1]
                SPECULAR_LIGHT[1] = cmd[2]
                SPECULAR_LIGHT[2] = cmd[3]
