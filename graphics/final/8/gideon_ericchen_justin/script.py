...
def first_pass( commands ):
    global basename, num_frames, isanim
    num_frames = 1
    isanim = False
    vary = False
    frames = False
    ...
    if vary and not frames:
        sys.exit()
    elif frames and basename=="":
        basename = "default"
        print "No basename found. Using default name 'default'"
    if num_frames > 1:
        isanim = True
    ...
    for command in commands:
        if command[0] == "vary":
            for i in range(num_frames):
                if i < int(command[2]):
                    knobs[i][command[1]] = 0
                elif i > int(command[3]):
                    knobs[i][command[1]] = 1
                else:
                    num = 1.0 * int(command[4]) \
                        + i * (1.0 * (int(command[5]) - int(command[4]))) / \
                        (1.0 * num_frames)
                    knobs[i][command[1]] = num
    return knobs
...
def run(filename):
    ...
    first_pass(commands)
    knobs = second_pass(commands, num_frames) #Should be set in first_pass
    ...
    for i in range(num_frames):
        ...
        for command in commands:
            ...
            if command[0] == "move":
                ...
                yval = command[2]
                zval = command[3]
                if len(command) == 5 and command[4] in knobs[i]:
                    #if command[4] not in knobs[i]:
                        #print "Error with knobs"
                    #else:
                    c = knobs[i][command[4]]
                    xval *= c
                    yval *= c
            ...
            if command[0] == "rotate":
                ...
                if len(command) == 4 and command[3] in knobs[i]:
                    #if command[3] not in knobs[i]:
                        #print "Error with knobs"
                    #else:
                    angle *= knobs[i][command[3]]
        ...
        if isanim:
            save_extension(screen, "animation/%s%03d.png" % (basename,i))
