    ...
    while p < len( points ) - 2:
        ...
        if calculate_dot( points, p ) >= 0:
            ...
            draw_line( screen, points[p][0], points[p][1],points[p][2],
                       points[p+1][0], points[p+1][1],points[p+1][2], It, zbuff )
            draw_line( screen, points[p+1][0], points[p+1][1],points[p+1][2],
                       points[p+2][0], points[p+2][1],points[p+2][2], It, zbuff )
            draw_line( screen, points[p+2][0], points[p+2][1], points[p+2][2],
                       points[p][0], points[p][1],points[p][2], It, zbuff )
            scanline_convert(points[p][0], points[p][1],points[p][2],
                       points[p+1][0], points[p+1][1],points[p+1][2], points[p+2][0],points[p+2][1], points[p+2][2], screen, It, zbuff)
        p+= 3


def scanline_convert( x1,y1,z1,x2,y2,z2,x3,y3,z3, screen,color, zbuff):
    ...
    while y<ty:
        ...
        xb+=d1
        y+=1
        draw_line(screen,int(xa),int(y), z1, int(xb),int(y), z2, color, zbuff)  #z is impercise
...
def z_scale(x0, y0, x, y, x1, y1, z0, z1, z_buffer):
    ...
    if (z_buffer[int(x)][int(y)] < z_value):
        z_buffer[int(x)][int(y)] = z_value
        return True
    return False
...
def draw_line( screen, x0, y0, z0, x1, y1, z1, color, z_buffer ):
    ...
    if (dx == 0 and dy == 0):
        ...
        y = y0
        while y <= y1:
            if z_scale(x0, y0, x0, y, x1, y1, z0, z1, z_buffer):
                plot(screen, color, x0, y)
            y = y + 1
    elif dy == 0:
        x = x0
        while x <= x1:
            if z_scale(x0, y0, x, y0, x1, y1, z0, z1, z_buffer):
        ...
        y = y0
        while x <= x1:
            if z_scale(x0, y0, x, y, x1, y1, z0, z1, z_buffer):
                plot(screen, color, x, y)
            if d > 0:
                y = y - 1
                d = d - dx
