...
def draw_polygons( points, screen, color ):
    ...
    while p < len( points ) - 2:
        ...
        if calculate_dot( points, p ) >= 0:
            ...
            order = [a1,a2,a3]
            orderct = 0
            while orderct < len( order ) - 1:
                if order[orderct][1] < order[orderct+1][1]:
                    holder = order[orderct]
                    order[orderct] = order[orderct+1]
                    order[orderct+1] = holder
                    orderct = -1
                orderct += 1
            ...
            difftb = top[1] - bot[1]
            if difftb != 0:
                c0 = (top[0] - bot[0]) / difftb
            else:
                c0 = 0
                
            difftm = top[1] - mid[1]
            if difftm != 0:
                c1tm = (top[0] - mid[0]) / difftm
            else:
                c1tm = 0
            
            diffmb = mid[1] - bot[1]
            if diffmb != 0:
                c1mb = (mid[0] - bot[0]) / diffmb
            else:
                c1mb = 0
            ...
            if diffmb != 0:
                ...
                while fillct < diffmb:
                    ...
                    x2 += c1mb
                    y += 1
                    draw_line(screen, x1, y, x2, y, color)
                    fillct += 1
