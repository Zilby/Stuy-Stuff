...
def scanline_convert(screen, x0, y0, z0, x1, y1, z1, x2, y2, z2, color):
    ...
    while (my >= n):
        d1 = ((n - by) / (ty - by)) * (tz - bz)
        d2 = ((n - by) / (my - by)) * (mz - bz)
        bx1 = bx + float(tx - bx) / (ty - by) * (n - by)
        bx2 = bx + float(mx - bx) / (my - by) * (n - by)
        draw_line(screen, bx1, n, bz + d1, bx2, n, bz + d2, color)
        n += 1
    while (ty >= n):
        d1 = ((n - by) / (ty - by)) * (tz - bz)
        d2 = ((n - my) / (ty - my)) * (tz - mz)
        bx1 = bx + float(tx - bx) / (ty - by) * (n - by)
        bx2 = mx + float(tx - mx) / (ty - my) * (n - my)
        draw_line(screen, bx1, n, bz + d1, bx2, n, mz + d2, color)
        n += 1
...
def draw_lines( matrix, screen, color ):
    ...
    while p < len( matrix ) - 1:
        draw_line( screen, matrix[p][0], matrix[p][1], matrix[p][2], matrix[p+1][0], matrix[p+1][1], matrix[p][2], color )
        p+= 2
