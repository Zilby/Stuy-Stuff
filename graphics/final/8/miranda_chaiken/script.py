    ...
    for command in commands:
        ...
        if cmd == "frames":
            ...
            has_anim = True
                ...
                ls.append([command[1],command[2],command[3]],[command[4],command[5],command[6]])

        elif command[0] == "ambient":
            a= [command[1],command[2],command[3]]
        
        elif command[0] == "constants":
            constantd[command[1]] = [[command[2],command[5],command[7]],[command[3],command[6],command[8]],[command[4],command[7],command[9]]]

                
    if (not num_frames == 1) and basename == "":
...
colors = [ [255,0,0],[0,255,0],[0,0,255],[100,0,150],[100,100,100]]
...
def run(filename):
    ...
    a= [0, 0, 255]
    ls = [ [[55, 0, 0], [10, 10, -10]]  ]
    constantd = {"tardis":[(.5,.5,.5),(.2,.2,.2),(.3,.3,.3)]}
    """
    ...
    for frame in range(0,num_frames):
        for command in commands:
            ...
            if command[0] == "sphere":
                m = []
                lconstant= constantd[command[1]]
                add_sphere(m, command[2], command[3], command[4], command[5], 5)
                matrix_mult(stack[-1], m)
                #lp=[command[2]-10, command[3]-10, command[4]-100]
                print lconstant
                draw_polygons( m, screen, color,zbuff,lconstant[0],lconstant[1],lconstant[2],lconstant[3],ls )
