...
def calc_color( points, i ):
    ...
    ambient = [ int(val*Ka) for val in Ia ]
    print "Ambient: ", str(ambient)

    dot = dot_product_light(N, L)
    diffuse = [ int(val*Kd*dot) for val in Ip ]
    print "Diffuse: ", str(diffuse)

    color = [ ambient[i] + diffuse[i] for i in range(3) ]
    #color = ambient
