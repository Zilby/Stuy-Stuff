...
def first_pass( commands ):
    for command in commands:
        ...
        if command[0] == "vary":
            if num_frames == 0:
                print "must set number of frames!"
                return
    if basename == "":
        basename = "anim"
        print "you did not set a unique basename. 'anim' has been used instead."
...
def second_pass(commands):
    ...
    for command in commands:
        if command[0] == "vary":
            ...
            end_value = command[5]
            step = 1.0 * (end_value - start_value) / (end_frame - start_frame)
            for i in range(num_frames):
                if i < start_frame:
                    knobs[i][name] = 0
                elif i > end_frame:
                    ...
                    knobs[i][name] = start_value + (step * (i - start_frame))
