...
def first_pass( commands ):
	global num_frames, name, anim
	f, b, v = 0, 0, 0
	for c in commands:
		if c[0] == 'frames' and not f:
			num_frames = c[1]
			f = 1
		elif c[0] == 'basename' and not b:
			name = c[1]
			b = 1
		elif c[0] == 'vary':
			v = 1
...
def second_pass( commands, num_frames ):
	...
	for c in commands:
		if c[0] == 'vary':
			knob, sframe, eframe, svalue, evalue = c[1], c[2], c[3], c[4], c[5]
			dvdf = float(evalue - svalue) / (eframe - sframe)
			for i in range(sframe):
				knobs[i][knob] = svalue
			for i in range(sframe, eframe):
				knobs[i][knob] = svalue + (i - sframe)*dvdf
			for i in range(eframe, num_frames):
				knobs[i][knob] = evalue
	return knobs
    ...
    for i in range(num_frames):
	    ...
	    for command in commands:
	        ...
	        if command[0] == "move":                
	            ...
	            yval = command[2]
	            zval = command[3]
		    if anim and len(command) == 5 and command[4] in knobs[i]:
		    ...
		    if anim and len(command) == 4 and command[3] in knobs[i]:
			angle *= knobs[i][command[3]]
	            ...
	            stack[-1] = t
	    if anim:
		save_extension(screen, name + ("%04d.png"%(i,)))
