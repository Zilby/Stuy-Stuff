...
/*======== void first_pass()) ==========
  ...
  ====================*/
void first_pass() {
	int varyf=0; //vary found
	int framef=0; //frame found
	int basef=0; //base found
	for (i=0;i<lastop;i++) {
      switch (op[i].opcode) {
		case VARY:
			varyf=1;
	...
	if (framef && !basef)
		printf("Since you have not provided a basename, \"base\" will be used.\n");
	if (varyf && !framef)
		exit(1);	
...
struct vary_node ** second_pass() {
	...
	char knobname[128];
	int switched;
	struct vary_node ** knobs=malloc(sizeof(*vary_node) * (num_frames));
	for (i=0;i<lastop;i++) { 
		switch (op[i].opcode) {
			case VARY:
				strcpy(op[i].op.vary.p->s->name,knobname);
				inc=(op[i].op.vary.end_val- op[lastop].op.vary.start_val)/(op[i].vary.end_frame-op[i].vary.start_frame);
				for (k=op[i].vary.start_frame;k<=op[i].vary.end_frame;k++){
					if (!knobs[k]){
						strcpy(knobname,tmp.name);
						tmp.value=inc*k;
					}
					switched=0;
					while (tmp=tmp->next && !switched){
						if (!strcmp(tmp.name,knobname)){
							tmp.value=inc*k;
							switched=1;
						}
					}
