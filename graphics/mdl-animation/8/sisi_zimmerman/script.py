...
def first_pass( commands ):
    ...
    ##     if command[0] = "vary":
    ##         if num_frames == -1:
    ##             print "cannot use vary without setting frames"
    ##             #break?
    ##             return 0
            
    ## if( num_frames != -1 && basename == ""):
    ##     basename = "foo"
    ##     print "no base name given, base name set to foo"
...
def second_pass( commands, num_frames ):
    ...
    for command in commands:
        ...
        if command[0] == "vary":
            ...
            step = (command[5] - command[4])/float((start - end))
            val = command[4]-step
            
            for x in range(num_frames):    
                if (x >= start  and x <= end):
                    knobs[x][command[1]]= val + step
                    val += step
                else:
                    knobs[x][command[1]]= val
    return knobs
...
def run(filename):
    ...
    while i < num_frames:
        ...
        #out of for loop of commands, in while loop of frames
        i +=1
        name = basename+"%03d"%i+".png"
