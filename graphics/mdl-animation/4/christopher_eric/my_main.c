...
void first_pass() {
  ...
  for (i=0;i<lastop;i++){
    ...
    case VARY:
      if (! FRAMES_FOUND)
	perror("lol you didnt give frames");
    }
  }
}
...
struct vary_node ** second_pass() {
  ...
  struct vary_node **ary;
  for (i=0;i<lastop;i++)//find how many varies there are
    if (op[i].opcode == VARY)
      size++;
  ary=  calloc(size,sizeof(struct vary_node*));
  for (i=0;i<lastop;i++){
    if (op[i].opcode == VARY){
      //grab stuff :^)
      ...
      char namerino[128];
      strncpy(namerino,op[i].op.vary.p->name,128);
      //begin the nodes.
      int knob_index;
      struct vary_node* knob,* prev,* head;
      for (knob_index=0;knob_index<num_frames;knob_index++){
	//create each knob
	knob = calloc(1,sizeof(struct vary_node));
	strncpy(knob->name,namerino,128);
	//====index===start_____________end___________
	knob->value = start_val;
	if (knob_index >= start_frame && knob_index<=end_frame){
	  //========start_____index____end=============
	  knob->value += (end_val-start_val)/(end_frame-start_frame);
	}else if (knob_index > end_frame){
	  //=======start______________end=====index===
	  knob->value = end_val;
	}
	if (knob_index==0){
	  //keep track of head
	  ...
	  //just keep adding next knob node
	  prev->next = knob;
	  prev = knob;
	}
      }
      //move on to the next VARY frame
      ary[frame_num]=head;
      frame_num++;
    }
  }
