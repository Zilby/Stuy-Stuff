...
def first_pass( commands ):
    ...
    for command in commands:
        if command[0] == "frames" and frames == 0:
            if len(command[1]) > 0:
                frames = command[1]
        if command[0] == "basename" and len(basename) == 0:
            if len(command[1]) > 0:
                basename = command[1]
        if command[0] == "vary": 
            counter++
    if frames == 0 and counter > 0:
        exit(0)
    if frames > 0 and len(basename) == 0:
        exit(0)
    return frames
...
def run(filename):
    ...
    num_frames = 0
    basename = ""
    counter = 0 #just keeps track of how many times vary is in the command list
    if p:
        (commands, symbols) = p
    else:
    ...
    for command in commands:
        ...
        if command[0] == "rotate":
            ...
            matrix_mult( stack[-1], t )
            stack[-1] = t
        # FIRST PASS IN CHECKING ANIMATION
        #first_pass (commands)
        #... going through the whole command array again is sort of ineefficient (increasing runtime)
        if command[0] == "frames" and frames == 0: #takes first frame command and ignores all other commands
            if len(command[1]) > 0:
        ...
        if command[0] == "vary": 
            counter++
        if num_frames == 0 and counter > 0:
            exit(0)
        if num_frames > 0 and len(basename) == 0:
            exit(0)
