...
public class gui implements ActionListener,MouseListener, MouseMotionListener {
	...
	if ((e.getModifiers() & InputEvent.BUTTON3_MASK)
	    ...
	    z = Math.sqrt(x*x + y*y);
	    z = Math.floor(z);
	    canvas.addLine(xes[0],ys[0],0,xes[0],ys[0],(int)z);
	}
	else{
	    int x,y;
	    x = e.getX();
	    ...
	    canvas.stopDrawing();
	    System.out.println(canvas.getEdges());
	}
    }
	...
	if (e.getSource()==quit) {
	    ...
	    if (transformation.getSelectedItem() == "scale")
		canvas.scale(Double.parseDouble(xarg.getText()),
			     Double.parseDouble(yarg.getText()),
			     Double.parseDouble(zarg.getText()));
	    if (transformation.getSelectedItem() == "translate")
		canvas.translate(Double.parseDouble(xarg.getText()),
				 Double.parseDouble(yarg.getText()),
				 Double.parseDouble(zarg.getText()));
	    if (transformation.getSelectedItem() == "x rotation"){
		double theta = Double.parseDouble(xarg.getText());
		canvas.rotX(theta);
	    }
	    if (transformation.getSelectedItem() == "y rotation"){
		double theta = Double.parseDouble(yarg.getText());
		canvas.rotY(theta);
	    }
	    if (transformation.getSelectedItem() == "z rotation"){
		double theta = Double.parseDouble(zarg.getText());
		canvas.rotZ(theta);
	    }
	...
	System.out.println(g.canvas.getEdges());	
    }
}
