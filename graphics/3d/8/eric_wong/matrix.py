...
def make_sphere(phi):
    ...
    ident(s)
    s[0] = [1, 0, 0, 0]
    s[1] = [0, math.cos(phi), -1*math.sin(phi), 0]
    s[2] = [0, math.sin(phi), math.cos(phi), 0]
    s[3] = [0, 0, 0, 1]
    return s
...
def make_torus(phi):
    t = new_matrix()
    ident(s)
    t[0] = [math.cos(phi), 0, -1*math.sin(phi), 0]
    t[1] = [0, 1, 0, 0]
    t[2] = [math.sin(phi), 0, math.cos(phi), 0]
    t[3] = [0, 0, 0, 1]
    return t
...
def generate_curve_coefs( p1, p2, p3, p4, t ):
    if t == "hermite":
        h = matrix_mult(make_hermite(), [[p1, p2, p3, p4]])
        return h
    if t == "bezier":
        b = matrix_mult(make_bezier(), [[p1, p2, p3, p4]])
        return b
    else:
