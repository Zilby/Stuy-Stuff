...
from matrix import *
import math
def add_rect(points, x,y,z,l,w,d):
    add_edge(points,x,y,z,x,y,z)

    add_edge(points,x+w,y,z,x+w,y,z)
    add_edge(points,x,y-l,z,x,y-l,z)
    add_edge(points, x+w,y-l,z,x+w,y-l,z)


    add_edge(points,x,y,z-d,x,y,z-d)
    
    add_edge(points,x+w,y,z-d,x+w,y,z-d)
    add_edge(points,x,y-l,z-d,x,y-l,z-d)
    add_edge(points, x+w,y-l,z-d,x+w,y-l,z-d)
...
def add_sphere(points,cx,cy,cz, r, step):
    ...
    for p in range(0,101,step):
        for c in range (0,101,step):
            x = r*math.cos(2*math.pi*(c/100.0))+cx
            y = r*math.sin(2*math.pi*(c/100.0))*math.cos(math.pi*(p/100.0))+cy
            z = r*math.sin(2*math.pi*(c/100.0))*math.sin(math.pi*(p/100.0))+cz
            add_edge(points,x,y,z,x,y,z)
    print points

def add_penne(points,cx,cy,cz,r,R,step):
    step= int(step*100)
    for p in range(0,101,step):
        for c in range (0,101,step):
            x = math.cos(2*math.pi*(p/100.0)) * (r*math.cos(2*math.pi*(c/100.0))+R) + cx
            y = r * math.sin(2*math.pi*(c/100.0)) + cy
            z = math.cos(2*math.pi*(p/100.0)) * (r*math.cos(2*math.pi*(c/100.0))+R) + cz
            add_edge(points,x,y,z,x,y,z)
...
def add_torus(points,cx,cy,cz,r,R,step):
    ...
    for p in range(0,101,step):
        for c in range (0,101,step):
            x = math.cos(2*math.pi*(p/100.0)) * (r*math.cos(2*math.pi*(c/100.0))+R) + cx
            y = r * math.sin(2*math.pi*(c/100.0)) + cy
            z = math.sin(2*math.pi*(p/100.0)) * (r*math.cos(2*math.pi*(c/100.0))+R) + cz
            add_edge(points,x,y,z,x,y,z)
