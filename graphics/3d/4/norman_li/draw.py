...
def add_sphere( points, cx, cy, cz, r, step ):
    ...
    for p in range(rot):
        for c in range(rot):
            x = cx + r * cos( 2 * pi * step * c )
            y = cy + r * sin( 2 * pi * step * c ) * cos( pi * step * p )
            z = cz + r * sin( 2 * pi * step * c ) * sin( pi * step * p )

            add_point( points, x, y, z )

# DD(TM) Donut Function :)
def add_torus( points, cx, cy, cz, r1, r2, step ):
    ...
    for p in range(rot):
        for c in range(rot):
            x = cx + cos( 2 * pi * step * p ) * ( r1 * cos( 2 * pi * step * c ) + r2 )
            y = cy + r1 * sin( 2 * pi * step * c )
            z = cz + sin( 2 * pi * step * p ) * ( r1 * cos( 2 * pi * step * c ) + r2 )
