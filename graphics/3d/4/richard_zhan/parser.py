    ...
    while c  <  len(commands):
        ...
        if cmd in 'lstxyzcbhpmdw':
            ...
            if cmd == 'l':
                ...
                add_sphere( points, args[0], args[1], args[2], .05, .01)
            elif cmd == 'd':
                add_torus( points, args[0], args[1], args[3], args[2], .05, .01) 
            elif cmd == 'c':
            ...
            points = []
            screen = new_screen()
            #am i suppose to be clearing the screen
        else:
...
#parse_file("circle",points,transform)
