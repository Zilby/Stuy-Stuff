...
def add_circle( points, cx, cy, cz, r, step ):
    ...
    zi = cz
    while t < 1.0+step:
        x = r*math.cos(math.pi*t*2) + cx
        y = r*math.sin(math.pi*t*2) + cy
        z = 0
        if t != 0:
            add_edge(points,xi,yi,zi,x,y,z)
        xi = x
        yi = y
        zi = z
        t += step
...
def add_sphere( points, cx, cy, r ):
    ...
    while t < 1.0+step:
        while s < 1.0+step:
            x = r*math.cos(math.pi*s*2) + cx
            y = r*math.sin(math.pi*s*2)*math.cos(math.pi*t) + cy
            z = r*math.sin(math.pi*s*2)*math.sin(math.pi*t) + cz
...
def add_torus( points, cx, cy, r1, r2 ):
    ...
    while t < 1.0 + step:
        while s < 1.0 + step:
            x = math.cos(math.pi*t*2)*(r1*math.cos(math.pi*2*s) + r2)+cx
            y = r1*math.sin(math.pi*s*2)+cy
            z = -1*math.sin(math.pi*t*2)*(r1*math.cos(math.pi*s*2) +r2)+cz
            add_point(points, x, y, z)
            add_point(points, x, y, z)
            s += step
        s = 0
